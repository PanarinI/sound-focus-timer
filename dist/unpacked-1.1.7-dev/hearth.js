...
const el = {};
['room', 'wash', 'star', 'sparks', 'timeslider', 'timerow', 'ticks', 'num', 'wrap', 'stage', 'home', 'pip', 'volknob', 'volarc', 'voldot', 'fast', 'premium', 'harmony', 'finish', 'settoggle', 'settings', 'sndhint', 'sndwave', 'sndmute', 'glowrow', 'glowtoggle', 'colourseg', 'ratebar', 'rstars',
 'startbtn', 'mixknob', 'mixarc', 'mixdot', 'energyknob', 'energyarc', 'energydot', 'maskknob', 'maskarc', 'maskdot']
...
function dropEmber(focusSec) {
  ...
  // счёт ЗАЧТЁННЫХ рейсов — топливо просьбы об оценке (карточка поверх звезды, паттерн ExportGPT).
  // Тычок в звезду «послушать, что это» рейсом не считается: ExportGPT просит после УДАЧНОГО экспорта,
  // здешний эквивалент удачи — рейс, в котором тебя реально несли (≥ ASK_MIN_MIN «минуты»; в fast — секунды).
  if (min >= ASK_MIN_MIN) {
    localStorage.setItem('hearth.sessions', String(+(localStorage.getItem('hearth.sessions') || 0) + 1));
    if (askReady()) { askArmed = true; setTimeout(syncAsk, 0); }    // придёт в покое; render — второй заход
    else setTimeout(syncAsk, 0);                                     // плашка могла открыться этим же рейсом
  }
  renderEmbers();
}
...
// ---------- ПРОСЬБА ОБ ОЦЕНКЕ (реш. автора 07-22 · переделана 08-09 под паттерн ExportGPT) ----------
// Одна воронка, два места: тихий ряд в доке (дверь для того, кто сам захотел) и КАРТОЧКА ПОВЕРХ ЗВЕЗДЫ.
// Карточка приходит ТОЛЬКО в покое, по прибытии, с ASK_AFTER-го зачтённого рейса — как «Rate us» на
// кнопке Export у ExportGPT: пока висит, обойти нельзя (клик по небу её СБРАСЫВАЕТ, а не поднимает рейс),
// но и не заперто — после следующего рейса просьба вернётся. Первый ответ звездой закрывает НАВСЕГДА.
// ≥4★ → отзывы CWS · 1–3★ → форма фидбека (боль ловим себе, не в стор).
// ДВА ГРОМКОСТИ ОДНОЙ ПРОСЬБЫ (реш. автора 08-10): тихая плашка в углу дока — ДВЕРЬ, открыта с первого
// прибытия для того, кому зашло сразу; карточка поверх звезды — ПРОСЬБА, приходит с третьего.
const ASK_AFTER = 3;      // карточка: первые два рейса — знакомство с интерфейсом, в них не просим (08-09)
const BAR_AFTER = 0;      // плашка: видна СРАЗУ, с первого открытия панели (реш. автора 08-10) — дверь
                          // не должна открываться по расписанию: захотел сказать — говори сейчас
const ASK_MIN_MIN = 1;    // рейс зачтён, если несомого времени ≥ 1 «минуты» (в fast-режиме = 1 секунда)
...
const rateOpen = () => !!(RATE_URL || FEEDBACK_URL) && !localStorage.getItem('hearth.rated');   // просьба вообще жива?
const askReady = () => rateOpen() && flights() >= ASK_AFTER;    // карточка
const barReady = () => rateOpen() && flights() >= BAR_AFTER;    // плашка

let askArmed = false, askCard = null, askVeil = null;

// одна лента звёзд — и в доке, и в карточке (ховер подсвечивает до курсора, клик отвечает и закрывает)
...
function answerRate(n) {
  ...
  localStorage.setItem('hearth.rated', '1');          // ответил один раз — не спрашиваем больше никогда
...
// Карточка строится по первой надобности: в разметке её нет, поэтому публичный стенд (шим hearth.rated='1')
// её даже не собирает — форкать логику не пришлось.
function buildAsk() {
  ...
  document.body.append(askVeil, askCard);
}
function dismissAsk() { askArmed = false; syncAsk(); }   // до следующего прибытия

// Одна просьба на экране: карточка перебивает тихий ряд, ряд остаётся дверью после сброса.
function syncAsk(phase) {
  const rest = (phase || engine.phase || 'off') === 'off';
  ...
  el.ratebar.hidden = !(rest && barReady() && !up);   // плашка уступает карточке: одна просьба на экране
}

// ---------- ЖИВАЯ КАЙМА ЗВЕЗДЫ (реш. автора 08-27) ----------
// «Идеально ровный шар» автор назвал лысым. На стенде `lab/star-form-panels.html` он выбрал из пяти
// форм ту, где различие ПОЧТИ НЕЗАМЕТНО: заметная неровность читается уже как клякса, а не звезда.
// Поэтому не лучи и не корона, а слегка гуляющий край. Восемь радиусов границы ведут несоизмеримые
// волны — тот же закон, что у дрейфа звука: рисунок не повторяется, глазу не за что зацепиться.
// Амплитуда ±3.5 %: в покое (звезда 16px) это доли пикселя, к прибытию (166px) — пара пикселей.
(function livingEdge() {
  if (!el.star) return;
  if (window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
  const P = 1.6180339887;
  const v = (t, k, ph) => (50 + 3.5 * Math.sin(t / k + ph)).toFixed(2) + '%';
  (function tick(ms) {
    const t = ms / 1000;
    el.star.style.borderRadius =
      `${v(t, 11, 0)} ${v(t, 11 * P, 1.2)} ${v(t, 13, 2.4)} ${v(t, 13 * P, 3.6)} / ` +
      `${v(t, 12, 0.7)} ${v(t, 12 * P, 1.9)} ${v(t, 14, 3.1)} ${v(t, 14 * P, 4.3)}`;
...
function render(st) {
  ...
  syncAsk(phase);                                          // просьба живёт ТОЛЬКО в покое — рейс не трогаем
  ...
  if (phase !== lastPhase) {
    ...
    paintNum(phase); syncHint(phase);            // цифра завода и подсказка живут только в покое
    lastPhase = phase;
  }
...
// ---------- ЧИСЛО ЗАВОДА И ПОДСКАЗКА ПЕРВОГО ЗАПУСКА (08-24, по голосу 2 «wheres the timer») ----------
// В ПОКОЕ цифра завода висит постоянно — это единственное на первом экране, что говорит «таймер».
// В полёте её нет: закон 07-25 («показываем оставшееся расстояние, а не утекающее время») цел, он
// написан про полёт. Пока тянут ползунок — цифра вспыхивает ярче и оседает обратно в покойную.
const dialText = () => (infinite ? '∞' : dialMin + '′');
// ПЕРЕДАЧА НА ВЗЛЁТЕ (08-24, реш. автора): цифра держится HANDOVER секунд после старта и тает —
// «летим, пятнадцать минут» сказано и отпущено. Это НЕ отсчёт: число не убывает, оно называет длину
// рейса один раз. Полный отсчёт на весь полёт не кладём — он вернул бы фрейм «терпишь», от которого
// ушли 07-25; сколько осталось, показывают звезда и дуга на иконке.
const HANDOVER_S = 4.5;
let handoverT = 0;
function paintNum(phase) {
  const rest = (phase || engine.phase || 'off') === 'off';
  if (!rest && Date.now() < handoverT) return;        // передача идёт — раньше времени не гасим
  el.num.textContent = dialText();
  el.num.style.opacity = rest ? 0.75 : 0;
}
function handoverNum() {
  el.num.textContent = dialText();
  el.num.style.opacity = 0.75;
  clearTimeout(flashNum._t);
  handoverT = Date.now() + HANDOVER_S * 1000;
  flashNum._t = setTimeout(() => { handoverT = 0; paintNum(); }, HANDOVER_S * 1000);
}
function flashNum(text) {
  el.num.textContent = text; el.num.style.opacity = 1;
  clearTimeout(flashNum._t);
  flashNum._t = setTimeout(() => paintNum(), 1100);
}

// Кнопка START — пара к Finish. Видна ровно в тех фазах, где тап по экрану означает «взлёт»
// (та же проверка, что в hearthClick), и уступает место кнопке Finish, когда рейс пошёл.
const FLOWN_KEY = 'hearth.flown';

// ── СЧЁТЧИКИ (09-02). Шина — track.js, подключена мягко: нет файла → счётчиков нет, продукт цел.
// Разбор точек и цена решения — lab/metrics-plan.md.
...
function syncHint(phase) {
  if (!el.startbtn) return;
  const p = phase || engine.phase || 'off';
  el.startbtn.hidden = !(p === 'off' || p === 'ручей');
}
...
function showDial() { flashNum(dialText()); }   // вспыхнуть ярче на кручении, потом осесть в покойную
...
function killNow() {                                          // клик во время угасания: оборвать в тишину СЕЙЧАС
  const focus = pendingEmber != null ? pendingEmber : Math.max(0, elapsedS() - engine.DAWN);
  T('flight_end', { how: 'aborted', min: +(Math.max(0, focus) / unit()).toFixed(1), len: infinite ? 'inf' : dialMin });
  pendingEmber = null;
  dropEmber(Math.max(0, focus));
  engine.turnOff();
}
...
function start() {                                             // тап по звезде = ВЗЛЁТ (старт рейса + звук)
  localStorage.setItem(FLOWN_KEY, '1'); syncHint('ручей');     // взлетели — Start уходит, придёт Finish
  T('flight_start', { len: infinite ? 'inf' : dialMin });
  handoverNum();                                              // «летим, N минут» — и отпустили
  flashSound();
  engine.turnOn();
  ...
  if (p === 'off' || p === 'ручей') start();
    ...
    sleepTimer = setTimeout(() => {                           // забыл вернуться → очаг тихо уснул, слепок честен
      if (engine.phase === 'ниточка') {
        T('flight_end', { how: 'slept', min: +(Math.max(0, engine.pausedAt) / unit()).toFixed(1), len: infinite ? 'inf' : dialMin });
...
paintNum('off'); syncHint('off');            // панель открылась в покое: цифра и подсказка на месте сразу
...
['pointerdown', 'pointerup'].forEach((t) => el.startbtn.addEventListener(t, (e) => e.stopPropagation()));
el.startbtn.addEventListener('click', () => { if (engine.phase === 'off' || engine.phase === 'ручей') start(); });
...
function makeKnob(node, arc, dot, value, onInput) {
  ...
  // ОСЬ ВЫБИРАЕТ РУКА (08-24, голос 3: «хочется крутить влево-вправо, а не вверх-вниз»).
  // Раньше ручка слышала только вертикаль: тянешь вбок — не происходит НИЧЕГО, и человек решает,
  // что ручка сломана, а не что он тянет не туда. Теперь ось встаёт по первому заметному движению
  // (>4 px) и до конца перетаскивания не меняется — руку не дёргает на диагонали. Вверх и вправо
  // прибавляют. Кто привык к вертикали, не теряет ничего.
  let dragX = 0, dragY = 0, valFrom = 0, axis = null;
  node.addEventListener('pointerdown', (e) => {
    e.stopPropagation();                       // клик по ручке — не тап по небу (иначе взлёт)
    node.setPointerCapture(e.pointerId);
    dragX = e.clientX; dragY = e.clientY; valFrom = v; axis = null; node.focus();
  });
  node.addEventListener('pointermove', (e) => {
    if (!node.hasPointerCapture(e.pointerId)) return;
    const dx = e.clientX - dragX, dy = e.clientY - dragY;
    if (!axis) {
      if (Math.max(Math.abs(dx), Math.abs(dy)) < 4) return;    // дрожь пальца осью не считаем
      axis = Math.abs(dx) > Math.abs(dy) ? 'x' : 'y';
    }
    set(valFrom + (axis === 'y' ? -dy : dx) / 150);            // 150 px = весь ход
  });
...
const mixKnob = makeKnob(el.mixknob, el.mixarc, el.mixdot, 0, (v) => {
  ...
  mixSaveTimer = setTimeout(() => { store.set(MIX_KEY, v); T('setting_changed', { what: 'tone', to: +v.toFixed(2) }); }, 300);
});
store.get(MIX_KEY, 0).then((v) => { mixKnob.set(v); engine.setMix(v); });

// ---------- ЦВЕТ ШУМА (08-31) ----------
// Три кружка своего цвета правее ручки тона (слово автора 08-31). Дефолт brown и остаётся brown:
// продукт с первой секунды звучит тем, за чем пришли (тот же закон, что у ручки тона).
const COLOUR_KEY = 'hearth.colour';
const COLOURS = ['brown', 'pink', 'white'];
// Волна по полю в выбранном цвете: сцена отвечает на жест и возвращается к тёплому.
// Не играет при восстановлении из хранилища — открытие панели не должно вспыхивать.
const WASH = { brown: '#c98a4e', pink: '#e79ab5', white: '#dfe6f5' };
let washTimer = 0;
function washPulse(c) {
  if (!el.wash) return;
  el.wash.style.setProperty('--wash', WASH[c] || WASH.brown);
  el.wash.classList.remove('go'); void el.wash.offsetWidth; el.wash.classList.add('go');
  clearTimeout(washTimer);
  washTimer = setTimeout(() => el.wash.classList.remove('go'), 1500);
}
function paintColour(c) {
  el.colourseg.querySelectorAll('.cdot').forEach((b) => {
    const on = b.dataset.c === c;
    b.classList.toggle('on', on); b.setAttribute('aria-checked', String(on));
    b.tabIndex = on ? 0 : -1;                 // радиогруппа: в таб-порядке только выбранный
  });
}
el.colourseg.addEventListener('click', (e) => {
  const b = e.target.closest('.cdot'); if (!b) return;
  paintColour(b.dataset.c); engine.setColour(b.dataset.c); store.set(COLOUR_KEY, b.dataset.c);
  washPulse(b.dataset.c);
  T('setting_changed', { what: 'colour', to: b.dataset.c });
});
el.colourseg.addEventListener('keydown', (e) => {          // радиогруппа: стрелки листают цвета
  const d = { ArrowUp: -1, ArrowLeft: -1, ArrowDown: 1, ArrowRight: 1 }[e.key];
  if (!d) return;
  e.preventDefault();
  const cur = COLOURS.indexOf(el.colourseg.querySelector('.cdot.on')?.dataset.c || 'brown');
  const next = COLOURS[(cur + d + COLOURS.length) % COLOURS.length];
  paintColour(next); engine.setColour(next); store.set(COLOUR_KEY, next); washPulse(next);
  el.colourseg.querySelector(`.cdot[data-c="${next}"]`).focus();
});
// store.get числит значение в localStorage-ветке (+s) — строку это ломает, потому нормализуем здесь
store.get(COLOUR_KEY, 'brown').then((c) => {
  const v = COLOURS.includes(c) ? c : 'brown';
  paintColour(v); engine.setColour(v);
});
...
[['energy', 'energyknob', 'energyarc', 'energydot', 0.4],
 ['masking', 'maskknob', 'maskarc', 'maskdot', 0.45]].forEach(([k, node, arc, dot, def]) => {
  ...
  makeKnob(el[node], el[arc], el[dot], v0, (v) => {
    ...
    engine.setChar({ [k]: v });
    clearTimeout(charTimer[k]);
    charTimer[k] = setTimeout(() => T('setting_changed', { what: k, to: +v.toFixed(2) }), 400);

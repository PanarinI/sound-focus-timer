...
- [x] ⚙️ **Сборка больше не ручная — `bash lab/build-zip.sh`** (08-06). Белый список внутри скрипта;
      падает, если файла нет на диске или его нет в готовом архиве; существующий zip не перезаписывает
      без `--force`. Закрыт риск новых файлов `icon.js` и `tone.js`: оба подключены мягко, забытый файл
      не роняет расширение — продукт просто молча едет без иконки-таймера и без тона.
- [ ] ⚙️ **Билд 1.1.0 собран** — `dist/minimalist-timer-1.1.0.zip` (232 КБ, 125 файлов, 53 локали).
- [x] 👤 VirusTotal — чисто (07-27, для 1.0.0). Для 1.1.0 прогнать заново.

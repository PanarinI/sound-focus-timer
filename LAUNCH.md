...
## Welcome Page — переделана 2026-09-02

Живёт в публичном репо `focus-pages/welcome/index.html`. **Правки лежат локально — до `git push`
живая страница показывает старую версию.**

**Что изменилось.**
1. **Второй кадр** — «как открыть после закрепления», со сквозной нумерацией (3). Канон `3.2`
   текстом его не требует, но ВСЕ три страницы, которые канон называет лучшими, устроены так:
   study-timer.pro прямо продолжает нумерацию («Open the extension (3) on any page»), у
   mp4-to-mp3 и heic-to-jpg вторая картинка помечена `alt="How to open"`. Без него человек
   закрепил иконку и не знает, что дальше — та же дыра, что дала голос 2 «wheres the timer».
   На кадре показана открытая боковая панель со звездой, ползунком и кружками цвета.
2. **Стрелки** — алые `#e2352a` вместо янтарных (сливались с белой панелью браузера), **прямые**,
   идут слева-сверху, вчетверо длиннее. Рисуются ОДНОЙ фигурой генератором на Python: голова —
   продолжение того же контура, а не `marker-end`. Прежде маркер масштабировался вместе с
   `stroke-width` (`markerUnits` по умолчанию = strokeWidth) и раздувался в «флажок» на огрызке.
3. **Иконка расширения** — настоящая искра из `lab/graphics/icon-master-bg.html` (путь + оба
   градиента), а не выдуманный кружок. Стоит в трёх местах через `<use>`.
4. **Пазл** — контур `extension` из репозитория Google (Material Symbols, viewBox `0 -960 960 960`)
   с **вырезанной нижней выемкой**: автор смотрит на живой Chrome и говорит, что её там нет.
   Сверка контуров — `lab/icon-compare.html`.
5. **Соседние иконки** в панели (3 приглушённых кружка) — у человека с расширениями пазл стоит в
   конце ряда, а не одиноко. Порядок выверен по скриншоту автора: пазл → разделитель → аватар → ⋮
   (иконки боковой панели там нет, стояла ошибочно).
6. Анимация расходящихся колец пробовалась и **отклонена**: на ExportGPT она работает потому, что
   там живая кнопка, а у нас картинка.

⚠️ **Урок метода.** Иконку нельзя «нарисовать по памяти» и нельзя обвести по скриншоту, где она
занимает 24 px. Брать надо из первоисточника: иконку продукта — из своего мастер-файла, чужую —
из репозитория владельца глифа. Если предел упирается в разрешение — говорить об этом сразу,
а не выдавать догадку за работу по референсу.

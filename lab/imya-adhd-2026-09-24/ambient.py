...
KEYS = [
    'ambient music', 'ambient sound', 'ambient sounds', 'ambient noise', 'ambient music for studying',
...
def main():
    ...
    json.dump(res, open(os.path.join(HERE, 'raw', 'volume_ambient.json'), 'w'), ensure_ascii=False)
    with open(os.path.join(HERE, 'spend.log'), 'a') as f:
        f.write(f"{datetime.datetime.now(datetime.timezone.utc):%Y-%m-%dT%H:%M:%SZ}\tambient\t${cost:.4f}\t{len(KEYS)} ключей, 4 года\n")
    rows = []
    for it in res:
        by = {}
    ...
    with open(os.path.join(HERE, 'ambient.csv'), 'w', newline='') as f:
